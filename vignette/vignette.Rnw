...
Version 1.0.4:
\begin{itemize}
\item The transition from one CNV state to another now takes into account the distance between exons (contribution from Anna Fowler and Gerton Lunter).
...
\item Addition of a function that uses a PCA algorithm to remove batch effect from the read count data (\texttt{correct.counts.using.PCA}). This step is experimental for now but is probably useful for gene panel experiments with a significant sample size.
...
\subsection{Principal component correction for batch effects}
The function \texttt{correct.counts.using.PCA} is so far an experimental function meant to remove batche effects using a principal component analysis strategy.
Look at the manual page for this function for more details, but so far the intended use are gene panel experiments (typically with a smaller number of targeted genes) with a relatively large sample size (greater than 20 seems appropriate but this number is not set in stone).
