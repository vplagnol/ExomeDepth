...
\item Addition of a function that uses a PCA algorithm to remove batch effect from the read count data (\texttt{correct.counts.using.PCA}). This step is experimental and should probably not be used unless you have some experience with that sort of correction and accept the potential side effects associated with it.
